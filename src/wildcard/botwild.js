...
export class TelegramWildcardBot {
  constructor(token, apiUrl, ownerId, cloudflareBotInstance) {
    this.token = token;
    this.apiUrl = apiUrl || 'https://api.telegram.org';
    this.ownerId = ownerId;
    this.cfBot = cloudflareBotInstance; // Instance of CloudflareBot
  ...
  async handleUpdate(update) {
    ...
    if (text.startsWith('/del ')) {
      ...
      let status;
      try {
        status = await this.cfBot.deleteSubdomain(subdomain);
      } catch (err) {
    ...
    if (text.startsWith('/list')) {
      let domains = [];
      try {
        domains = await this.cfBot.listSubdomains();
      } catch (err) {
